...
dest_add = r"D:\MS Documents\3rd Semester\Digital Image Processing\Assignment2 Solution"
...
source = r"D:\MS Documents\3rd Semester\Digital Image Processing\Assignment2\PH2Dataset\PH2 Dataset images"
...
for i in classes:
    ...
    # print(f"subfolder : {os.path.abspath(sub_fol)} \n number of files: {len(i['index'])}")
    counter = 0
    for j in i['index']:
        ...
        if os.path.isfile(from_add):
            ...
            shutil.copyfile(from_add, os.path.join(
                sub_fol, f'image{counter}.jpg'))
            counter += 1
        # if os.path.exists(os.path.join(source_directory, image_file)):

    # for j in i['index']:
    #   image = 'IMD'+j+'.BMP'
    #  source =
    # if os.path.exists(os.path.join(source, image)):